    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.success){
                ...
                $("#CN_ZDKT_ZZ").html(item.ZDKT_ZZ+"%");
                $("#CN_CUST_AUTOPN").html(item.CUST_AUTOPN);
                $("#CN_HAVE_CUST_AUTOPN").html(item.HAVE_CUST_AUTOPN);

                $("#CN_CUST_PRECRD").html(item.HAVE_CUST_PRECRD);
...
function queryMarketingStrategyData(){
    ...
    $.ajax({
        ...
        success: function (data) {
            if(data.success){
                var result = data.result;
                initChart(chart,"营销侧率分析",['常规投放增长','活动投放增长'], result.names,result.normals,result.activities,"%");
            }else{
                //失败
                window.parent.toastrError(data.errorMsg);
            }
...
function queryPersonNotYgChannelDataOnline(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.success){
                var result = data.result;
                initChartWithLine(chart,"非易购员工来源渠道",['查询期','对比期','增长率'],result.names,result.querys,result.compares);
            }else{
                //失败
                window.parent.toastrError(data.errorMsg);
            }
